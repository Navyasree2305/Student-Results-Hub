...
const StudentDashboard = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const lastSeen = localStorage.getItem('circulars_last_seen');
      const newCirculars = (data.items || []).filter(item =>
        item.type === 'circular' && (!lastSeen || new Date(item.created_at) > new Date(lastSeen))
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="card flex items-center space-x-4">
            ...
            <div className="text-left">
              ...
              <p className="text-2xl font-bold text-yellow-600">{cgpa !== null ? Number(cgpa).toFixed(2) : 'N/A'}</p>
            </div>
          </div>
        </div>
