...
def generate_timetable_excel(config, full_solution, metadata):
    ...
    for section, solution in sorted(full_solution.items()):
        # Extract section letter from scoped key e.g. "CSE_2_1_A" -> "A"
        sec_parts = section.rsplit("_", 1)
        sec_letter = sec_parts[-1] if len(sec_parts) == 2 else section
        ws = wb.create_sheet(title="Sec {}".format(sec_letter))
        _write_section_sheet(ws, config, solution, sec_letter, metadata, col_slots)
